...
#define EXAMPLE_ESP_AP_WIFI_SSID "ESP32"
#define EXAMPLE_ESP_AP_WIFI_PASS "76919716"
                                ...
                                int32_t event_id, void* event_data)
{
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        #ifdef DEVICE_HAS_UART
        char* Txdata = (char*) pvPortMalloc(100);
        sprintf (Txdata,"WIFIIP:" IPSTR, IP2STR(&event->ip_info.ip));
...
static void startAP(void) {
    ...
    #ifdef DEVICE_HAS_UART
    char* Txdata = (char*) pvPortMalloc(100);
    sprintf (Txdata,"APIP:" IPSTR, IP2STR(&if_info.ip));
    uart_write_bytes(UART, Txdata, strlen(Txdata));
    vPortFree(Txdata);
    #endif
}
...
void initialise_wifi(void) {
    ...
    char *ussid_name = "Heximpact";
    char *UPassword = "infi2022";

    // char *ussid_name = "OnePlus";
    // char *UPassword = "76919716";
    // ESP_ERROR_CHECK(nvs_open("Wifi_Config", NVS_READWRITE, &my_handle));
    // size_t urequired_size = 0;
    // esp_err_t nvsGetRet = nvs_get_str(my_handle, "USSID", NULL, &urequired_size);
    // if (nvsGetRet == ESP_OK && urequired_size) {
    //     ussid_name = (char *) pvPortMalloc(urequired_size);
    //     nvs_get_str(my_handle, "USSID", ussid_name, &urequired_size);
    //     ESP_LOGI(TAG,"Read USSIDdata: %s\n", ussid_name);

    //     size_t upaswdrequired_size;
    //     nvs_get_str(my_handle, "UPassword", NULL, &upaswdrequired_size);
    //     UPassword = (char *) pvPortMalloc(upaswdrequired_size);
    //     nvs_get_str(my_handle, "UPassword", UPassword, &upaswdrequired_size);
    //     ESP_LOGI(TAG,"Read upassword: %s\n", UPassword);
        
    //     nvs_close(my_handle);
    // } else {
    //     nvs_close(my_handle);
    //     ESP_LOGI(TAG, "SSID NOT SET YET");
    //     startAP();
    //     return;
    // }
...

    {
        ...
        * happened. */
        if (bits & WIFI_CONNECTED_BIT) {
            ...
            #ifdef DEVICE_HAS_SOCKET_AS_CLIENT
            initSockClient();
            #endif
...
void SetSSID(uint8_t *data, uint8_t *uint8_t_SSID, uint16_t len)
{
    ...
    if (SSIDtype == Default_SSID)
    {
        ...
        ESP_LOGI(TAG, "Write SSID: %s", SSID);
    ...
    else if (SSIDtype == User_SSID)
    {
        ...
        // nvs_handle_t my_handle;
        nvs_open("Wifi_Config", NVS_READWRITE, &my_handle);
        nvs_set_str(my_handle, "USSID", SSID);
        ESP_LOGI(TAG, "Write SSID: %s", SSID);
        nvs_commit(my_handle);
        nvs_close(my_handle);
...
void SetPSWD(uint8_t *data, uint8_t *uint8_t_upaswd, uint16_t len)
{
    ...
    if (Passwordtype == Default_SSID)
    {
        ...
        ESP_LOGI(TAG, "Write Password: %s", UPassword);
        nvs_commit(my_handle);
        nvs_close(my_handle);
    ...
    {   
        ...
        // nvs_handle_t my_handle;
        nvs_open("Wifi_Config", NVS_READWRITE, &my_handle);
        nvs_set_str(my_handle, "UPassword", UPassword);
        ESP_LOGI(TAG, "Write Password: %s", UPassword);
        nvs_commit(my_handle);
        nvs_close(my_handle);
    }
